...
while IFS= read -r line ; do
  ...
  if [[ "${line:0:1}" == "=" ]]; then
    ...
    cmd="$(echo $line | sed 's/^.*:://; s/##.*//; s/!!.*//')"
    usage="$(echo ${line%% ::*} | sed "s/\[/\x1b[37m\[/g; s/\]/\]$CWHI/g; s/^/$CMAG/; s/(/$CCYA(/; s/)/)$CWHI/; s/$/$CDEF/")"
    if echo $line | grep "##" > /dev/null; then 
        help="$(echo $line | sed "s/^.*## */$CGRE # /; s/$/\x1b[0m/")"
        line="${line%%##*}"
    else 
        help=""
    fi
    if echo $line | grep "!!" > /dev/null; then 
        comp_cmd="$(echo $line | sed 's/.*!!\(.*\)/\1/')"
        line="${line%%!!*}"
    else
        comp_cmd=""
    fi
    ...
    for word in $word_list; do
      if [[ "$word" == "[" ]]; then
        ...
        shortcut=${word/(/}
        shortcut=${shortcut/)/}
        alias_list+=$'\n'"alias @${shortcut}='$cli_name $shortcut'"
        alias_name_list+="@${shortcut} "
      else
    ...
    # Generate the auto complete option if one specified
    if [[ "$comp_cmd" != "" ]]; then
        comp_option_list+='
        if [[ "$all" == "'$key_words'" || "$prev" == "'$shortcut'" || "$prev" == "@'$shortcut'" ]]; then
            COMPREPLY=( $(compgen -W "$('$comp_cmd')" -- "$cur") )
        fi'
    fi
...
complete -F _'$cli_name'_complete '$cli_name' '$alias_name_list'
' > $cli_name.alias


echo '# Shortcut aliases' >> $cli_name.alias
echo "$alias_list" >> $cli_name.alias

    # if [[ "$prev" == "dsh" || "$prev" == "@dsh" ]]; then
